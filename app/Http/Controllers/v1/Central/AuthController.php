...
class AuthController extends Controller
{
    ...
    public function me(Request $request)
    {
        ...
        if ($tenant) {
            return $tenant->run(function () use ($hrData, $user, $tenantId) {
                $tenantUser = User::where('external_id', $user->external_id)->firstOrFail();
                $tenantUser->load(['roles', 'permissions']);

                $authData = [
                    'tenant_id'   => $tenantId,
                    'roles'        => $tenantUser->roles->pluck('name')->toArray(),
                    'permissions' => $tenantUser->permissions->pluck('name')->toArray()
                ];

                $redisKey = "user:{$user->external_id}:auth";
                Redis::connection('default')->del($redisKey);
                Redis::connection('default')->set($redisKey, json_encode($authData));
